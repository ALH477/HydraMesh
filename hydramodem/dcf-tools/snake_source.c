 ...
 *
 * PHY: audio is read as raw 32-bit-float mono @48k from a file / stdin / shared memory ring
 * (ALSA capture is a gated future edge); frames go out on a raw-L2 socket when --iface is
 * given, else the pipeline runs "dry" (batches counted) so the node builds and self-tests
 * without CAP_NET_RAW or quanta.
 *
 * Shared memory mode (--shm): reads from /demod-snake-tx ring buffer written by demod-rt
 * (Faust DSP engine). This is the spoke integration path for the RISC-V cluster.
 *
 * Usage: snake_source [--iface IF] [--channel NAME] [--node-id N] [--mode live|near|relaxed]
 *                     [--block N] [--in FILE] [--shm] [--frames N] [--no-quanta] [--selftest]
 */
...
typedef struct {
    ...
    int node_id, block, max_frames, no_quanta, selftest, shm;
} src_opts_t;

static void parse(int argc, char **argv, src_opts_t *o) {
    o->iface = NULL; o->channel = "snake"; o->infile = NULL; o->mode = "live";
    o->node_id = 0x00A1; o->block = 3072; o->max_frames = -1; o->no_quanta = 0; o->selftest = 0; o->shm = 0;
    for (int i = 1; i < argc; i++) {
        ...
        else if (!strcmp(argv[i], "--shm"))       o->shm = 1;
    }
...
/* Open shared memory ring buffer written by demod-rt.
 * Returns mapped SnakeSpsc pointer, or NULL on failure.
 * Caller must munmap + shm_unlink when done. */
static SnakeSpsc *open_tx_ring(const char *name, int *fd_out, size_t *size_out) {
    int fd = shm_open(name, O_RDWR, 0600);
    if (fd < 0) {
        fprintf(stderr, "snake-source: shm_open(%s) failed: %s\n", name, strerror(errno));
        return NULL;
    }
    size_t sz = snake_spsc_alloc_size(SNAKE_IPC_RING_CAP);
    void *ptr = mmap(NULL, sz, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if (ptr == MAP_FAILED) {
        fprintf(stderr, "snake-source: mmap failed: %s\n", strerror(errno));
        close(fd);
        return NULL;
    }
    *fd_out = fd;
    *size_out = sz;
    return (SnakeSpsc *)ptr;
}

static void close_tx_ring(SnakeSpsc *ring, int fd, size_t sz) {
    if (ring) {
        munmap(ring, sz);
        close(fd);
    }
...
int main(int argc, char **argv) {
    ...
    /* Open input source: file/stdin or shared memory ring */
    FILE *in = NULL;
    SnakeSpsc *tx_ring = NULL;
    int tx_fd = -1;
    size_t tx_size = 0;

    if (o.shm) {
        tx_ring = open_tx_ring(SNAKE_IPC_TX_SHM_NAME, &tx_fd, &tx_size);
        if (!tx_ring) {
            fprintf(stderr, "snake-source: failed to open TX ring %s\n", SNAKE_IPC_TX_SHM_NAME);
            return 1;
        }
        fprintf(stderr, "snake-source: reading from shared memory ring %s\n", SNAKE_IPC_TX_SHM_NAME);
    } else {
        in = o.infile ? fopen(o.infile, "rb") : stdin;
    ...
    for (;;) {
        size_t got = 0;
        if (o.shm) {
            /* Read from shared memory ring (non-blocking) */
            got = snake_spsc_pop(tx_ring, pcm, (uint64_t)o.block);
            if (got == 0) {
                /* Ring empty — brief sleep to avoid busy-wait, then retry */
                usleep(100);
                continue;
            }
        } else {
            /* Read from file/stdin */
            got = fread(pcm, sizeof(float), (size_t)o.block, in);
            if (got == 0) break;
        }
    ...
    free(pcm);
    if (o.shm && tx_ring) {
        close_tx_ring(tx_ring, tx_fd, tx_size);
    } else if (in != stdin) {
        fclose(in);
    }
