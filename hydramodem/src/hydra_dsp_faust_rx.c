 ...
 *   faust -lang c -os -double -ftz 2 -cn hydrarx -I faust \
...
void hydra_rx_dsp_process(hydra_rx_dsp *d, const float *audio_in, float *iq_out, int n)
{
    ...
    /* -os mode: precompute control-rate values (none for RX, but call for
     * interface correctness and forward-compatibility). */
