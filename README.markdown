...
- **Lisp**: SBCL with Quicklisp; dependencies: `cl-protobufs`, `cl-grpc`, `cffi`, etc. (see `lisp/src/d-lisp.lisp`).
- **StreamDB**: Build `libstreamdb.so` from `streamdb/` using Cargo for persistence in Lisp SDK.
...
- **Lisp**: Load via SBCL: `(load "lisp/src/d-lisp.lisp")`
...
# perl/dcf.pl
use Grpc::XS;
use DCF::Messages qw(DCFMessage);
my $client = Grpc::XS::channel('localhost:50051');
my $stub = $client->service('DCFService');
my $request = DCFMessage->new(data => 'Hello');
...
# python/dcf.py
import grpc
from dcf.services_pb2_grpc import DCFServiceStub
from dcf.messages_pb2 import DCFMessage
channel = grpc.insecure_channel('localhost:50051')
stub = DCFServiceStub(channel)
request = DCFMessage(data='Hello')
...
int main() {
    DCFClient* client = dcf_client_init("config.json");
    dcf_client_start(client);
    dcf_client_send(client, "Hello", "peer1");
...
class ServerImpl final : public DCFService::Service {
...
const dcfProto = grpc.loadPackageDefinition(packageDefinition).dcf;
const client = new dcfProto.DCFService('localhost:50051', grpc.credentials.createInsecure());
...
```lisp
;; lisp/src/d-lisp.lisp (excerpt)
(in-package :d-lisp)
(dcf-init "config.json" :restore-state t)
(dcf-start)
(dcf-quick-send "Hello from Lisp!" "localhost:50052")
(dcf-db-insert "/test/key" "test data")  ; Store in StreamDB
(print (dcf-db-query "/test/key"))  ; Query from StreamDB
(dcf-stop)
```
...
## Configuration
Create `config.json` based on `config.json.example`. DCF supports various optimization levels to balance performance, reliability, and resource usage:
...
- Perl: `prove -r tests/`
- Python: `pytest tests/`
- Lisp: `sbcl --eval '(d-lisp:run-tests)'`
