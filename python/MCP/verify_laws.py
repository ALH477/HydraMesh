...
def fields_from_bits(n: int):
    ...
    return f
enc_fields = [fields_from_bits(0)] + [fields_from_bits(1 << i) for i in range(108)]
enc_basis = [{"input_bits": "0" * 27, "frame": encode(**enc_fields[0]).hex()}]
for i in range(108):
    enc_basis.append({"input_bit": i, "frame": encode(**enc_fields[i + 1]).hex()})
syn_words = [bytes(17)]
syn_basis = [{"word": "00" * 17, "syndrome": syndrome(bytes(17))}]
for i in range(136):
    word = bytearray(17); word[i // 8] = 1 << (7 - i % 8)
    syn_words.append(bytes(word))
...
# Dependency-free C mirror of the certificate, so the C wire cert can diff against
# all 246 vectors without a JSON parser — the same golden_vectors.gen.h pattern every
# adapter already uses (codec/{superpack,fec,mesh,...}_vectors.gen.h).
def _carr(bs):
    return "{%s}" % ",".join(str(b) for b in bs)

hdr = os.path.join(os.path.dirname(os.path.abspath(out)), "golden_vectors.gen.h")
with open(hdr, "w") as hh:
    hh.write("/* GENERATED by python/MCP/verify_laws.py — DO NOT EDIT. */\n")
    hh.write("#ifndef DCF_GOLDEN_VECTORS_GEN_H\n#define DCF_GOLDEN_VECTORS_GEN_H\n")
    hh.write("#include <stdint.h>\n\n")
    hh.write("/* encode_basis: field inputs -> the exact 17 wire bytes. */\n")
    hh.write("typedef struct { uint8_t frame_type; uint16_t seq, src, dst;\n"
             "                uint8_t payload[4]; uint32_t ts_us; uint8_t frame[17]; } wire_encode_t;\n")
    hh.write("static const wire_encode_t WIRE_ENCODE[] = {\n")
    for f, v in zip(enc_fields, enc_basis):
        hh.write("  {%d,%d,%d,%d,%s,%dU,%s},\n" % (
            f["frame_type"], f["seq"], f["src"], f["dst"],
            _carr(f["payload"]), f["ts_us"], _carr(bytes.fromhex(v["frame"]))))
    hh.write("};\nstatic const int WIRE_ENCODE_N = "
             "(int)(sizeof(WIRE_ENCODE)/sizeof(WIRE_ENCODE[0]));\n\n")
    hh.write("/* syndrome_basis: a 17-byte word -> its affine validity syndrome. */\n")
    hh.write("typedef struct { uint8_t word[17]; uint16_t syndrome; } wire_syndrome_t;\n")
    hh.write("static const wire_syndrome_t WIRE_SYNDROME[] = {\n")
    for word, v in zip(syn_words, syn_basis):
        hh.write("  {%s,0x%04XU},\n" % (_carr(word), v["syndrome"]))
    hh.write("};\nstatic const int WIRE_SYNDROME_N = "
             "(int)(sizeof(WIRE_SYNDROME)/sizeof(WIRE_SYNDROME[0]));\n")
    hh.write("#endif\n")
print(f"  INFO  wrote C mirror: {hdr} ({os.path.getsize(hdr)} bytes)")
