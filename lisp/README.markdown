...
Version 1.8.0 | Updated: October 3, 2025Developed by DeMoD LLC with contributions from Grok 4 Heavy (xAI)License: GNU General Public License v3.0 (GPL-3.0)Repository: https://github.com/ALH477/DeMoD-Communication-FrameworkDemo: YouTube Video
Overview
...
Build StreamDB:Ensure libstreamdb.so is available (build from StreamDB’s Rust source or obtain precompiled binary).
...
Create a Configuration File (config.json):D-LISP supports three optimization levels in configuration to balance performance, reliability, and resource usage:
...
Quick Start:Initialize and start a client node with StreamDB:
(in-package :d-lisp)
...
Initialize with StreamDB: (dcf-init "config.json" :restore-state t) loads and validates the JSON configuration, restoring state from StreamDB at dcf.streamdb.
Set Mode and Start: (dcf-set-mode "p2p") and (dcf-start) configure and launch a P2P node.
Load Plugin: (dcf-load-plugin "lisp/plugins/lorawan-transport.lisp") adds LoRaWAN support in ~26 lines.
Add Middleware: A logging middleware customizes message processing.
Send/Receive Messages: Uses LoRaWAN for IoT communication, with messages logged to StreamDB under /messages/sent and /messages/received.
Visualize Topology: Generates a Graphviz DOT file for debugging.
Metrics and Failure Simulation: Monitors performance and tests self-healing.
StreamDB Operations: Stores and queries data, leveraging StreamDB’s reverse trie for efficient path-based access.
