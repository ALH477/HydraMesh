"""DCF TextLab core — a text adapter over the 17-byte DeModFrame wire quantum.

Chat text is an *adapter* over the wire quantum, exactly like DCF-Audio: a UTF-8
message is fragmented into ordinary DeModFrame frames whose 4-byte payloads carry
the bytes, with the fragment bookkeeping packed into the `seq` field.  The frames
are byte-for-byte valid DeModFrames (sync 0xD3, version nibble 1, CRC-16/CCITT),
so they satisfy the same wire invariant the 246-vector certificate pins — this
module reuses the certified codec in ``python/MCP/wirelab_core.py`` so nothing
about the quantum is reinvented here.

Why a separate adapter from audio:
  * Audio rides on CTRL frames (type 3).  Text rides on DATA frames (type 0), so a
    reassembler never confuses a voice fragment with a chat fragment.
  * Chat messages are larger and lower-rate than 20 ms audio blocks, so the seq
    split gives the fragment index more room (10 bits) than audio's 5.

L2 framing (all frames version=1, type=DATA(0), big-endian):
...
class TextReassembler:
    ...
    Emitted event: ("message", packet_id, ts_us, src, dst, text, flags)
    """
...
def _selftest():
    ...
    for pid, msg in enumerate(cases):
        ...
        # replaying any already-consumed data fragment must not double-emit
    ...
    print("dcf_text selftest: CERTIFIED")
