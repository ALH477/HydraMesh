...
- **Sensor telemetry over a wire (DCF-Sense)**: a configurable layer for many sensor nodes → one gateway over a wired audio-band HydraModem link (greenhouses, etc.). One reading = one bare frame (`src_id`=node, 4-byte scaled payload); a configurable **MAC** (TDMA slots / dedicated lines; FDMA/CSMA next) handles the shared medium since a PHY has none. An adapter over the quantum (certificate untouched). Phase-1 MVP (schema + MAC + node/gateway + demo, PER 0% on loopback) in `python/dcf/sense/`; see [`Documentation/DCF_SENSE_SPEC.md`](Documentation/DCF_SENSE_SPEC.md).
