...
use dcf_rust_sdk::{
    audio, game, reassemble_audio_payload, reassemble_game_payload, GameEvent, MessageHandler,
    Position,
};
...
/// A peer's latest position (dot-arena demo) — latest-wins per source.
#[derive(Serialize, Clone)]
pub struct UiGameSnapshot {
    pub src: u16,
    pub x: f32,
    pub y: f32,
    pub channel: u16,
...
/// A peer's discrete game action (opaque EVENT body decoded as UTF-8 text).
#[derive(Serialize, Clone)]
pub struct UiGameEvent {
    pub src: u16,
    pub text: String,
    pub channel: u16,
}
...
pub struct UiHandler {
    ...
    reasm: Mutex<HashMap<u16, audio::AudioReassembler>>, // per-source audio reassembly
    game_reasm: Mutex<HashMap<u16, game::GameReassembler>>, // per-source game reassembly
...
impl UiHandler {
    pub fn new(app: AppHandle, channel: Arc<AtomicU16>) -> Arc<Self> {
        Arc::new(Self {
            ...
            game_reasm: Mutex::new(HashMap::new()),
...
impl MessageHandler for UiHandler {
    ...
    fn handle_game(&self, data: &[u8], _from: SocketAddr) {
        if data.len() != 17 {
            return;
        }
        let my = self.channel.load(Ordering::Relaxed);
        ...
        let mut map = self.game_reasm.lock();
        let r = map.entry(src).or_default();
        if let Some(pkt) = reassemble_game_payload(r, &frame) {
            match pkt.msg_type_id {
                game::GMSG_SNAPSHOT if pkt.payload.len() == game::SNAPSHOT_LEN => {
                    let b: [u8; game::SNAPSHOT_LEN] = pkt.payload[..].try_into().unwrap();
                    let s = game::snapshot_unpack(&b);
                    let _ = self
                        .app
                        .emit("game-snapshot", UiGameSnapshot { src, x: s.x, y: s.y, channel: my });
                }
                game::GMSG_EVENT => {
                    let text = String::from_utf8_lossy(&pkt.payload).into_owned();
                    let _ = self
                        .app
                        .emit("game-event", UiGameEvent { src, text, channel: my });
                }
                _ => {}
            }
        }
